...
from keras.layers import (BatchNormalization, Concatenate, Conv2D, Dense,
                          Dropout, Flatten, Input, LeakyReLU, Reshape,
                          UpSampling2D)
...
D = Conv2D(32, (5, 5), strides=(2, 2), padding='same')(x_D)
...
workspace = 'bigan-mnist'
if not os.path.exists(workspace):
    os.makedirs(workspace)


def save(to):
    print('Saving {0}'.format(to))
    plot.savefig(to)
...
save('{0}/generative.png'.format(workspace))
...
for i in range(display_images):
    ...
    axis.get_xaxis().set_visible(False)
    axis.get_yaxis().set_visible(False)
save('{0}/reconstructions.png'.format(workspace))
...
save('{0}/latent-space.png'.format(workspace))
