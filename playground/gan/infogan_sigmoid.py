...
Q = Dense(latent_size)(D)
Q = Lambda(latent_activations)(Q)

P = Dense(1, activation='sigmoid')(D)


def mutual_information(prior_c, c_given_x, measure=K.binary_crossentropy):
    h_c = measure(prior_c, prior_c)
    h_c_given_x = measure(prior_c, c_given_x)
...
def sample_prior(size):
    ...
    continuous_1 = np.random.uniform(0, 1, size).reshape(-1, 1)
    continuous_2 = np.random.uniform(0, 1, size).reshape(-1, 1)
...
def plot_digits(filename, with_c1=False, with_c2=False):
    ...
    if with_c1:
        continuous_1 = np.linspace(0, 4, images_per_digit).reshape(-1, 1)
        continuous_1 = np.tile(continuous_1, (10, 1))
    else:
        continuous_1 = np.zeros([total_images, 1])
    if with_c2:
        continuous_2 = np.linspace(0, 4, images_per_digit).reshape(-1, 1)
...
os.mkdir('infogan-sigmoid')
plot_digits('infogan-sigmoid/fig_d.png')
plot_digits('infogan-sigmoid/fig_c1.png', with_c1=True)
plot_digits('infogan-sigmoid/fig_c2.png', with_c2=True)
