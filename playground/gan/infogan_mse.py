...
def latent_activations(Q):
    ...
    Q_continuous = Activation('tanh')(Q[:, -latent_continuous:])
...
def mean_squared_error(prior_c, c_given_x):
    return K.mean(K.sum(K.square(prior_c - c_given_x), axis=1))
...
def joint_mutual_information(prior_c, c_given_x):
    ...
    continuous = mean_squared_error(prior_c[:, -latent_continuous:],
                                    c_given_x[:, -latent_continuous:])
