...
def d_loss(_, logits):
    generated, real = tf.split(logits, 2)
    return K.mean(generated) - K.mean(real)
...
discriminator.compile(loss=d_loss, optimizer=RMSprop(lr=5e-5))

discriminator.trainable = False
gan = Model(z, discriminator(G))
gan.compile(loss=g_loss, optimizer=RMSprop(lr=5e-5))
        ...
        for batch_start in range(0, len(data) - batch_size + 1, batch_size):
            for _ in range(n_critic):
                ...
                for layer in discriminator.layers:
                    ...
                    clipped = [np.clip(w, -0.01, +0.01) for w in weights]
