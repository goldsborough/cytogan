...
# 100 x 1 x 1
G = Dense(7 * 7 * 128, activation='elu')(G)
G = Reshape((7, 7, 128))(G)
# 7 x 7 x 128
...
x_noisy = Lambda(add_noise)(x_flat)
E = Concatenate(axis=1)([x_noisy, c_D])
E = Dense(28 * 28 * 2, activation='elu')(E)
E = Reshape((28, 28, 2))(E)
# 28 x 28 x 1
...
data = mnist.images.reshape(-1, 28, 28, 1) * 2 - 1
labels = mnist.labels
...
def sample_images(filename, amount=25, rows=5, columns=5):
    noise = sample_noise(amount)
    conditional = sample_conditional(amount)
    images = generator.predict([noise, conditional])
...
try:
    for epoch in range(number_of_epochs):
        ...
        for batch_start in range(0, len(data) - batch_size + 1, batch_size):
            ...
            real_labels = labels[batch_start:batch_start + batch_size]

            noise = sample_noise(batch_size)
            fake_images = generator.predict([noise, real_labels])

            real_energy = discriminator.train_on_batch(
            ...
            noise = sample_noise(batch_size)
            fake_labels = sample_conditional(batch_size)
            g_loss = fake_energy = gan.train_on_batch(
                [noise, fake_labels], np.zeros_like(fake_images))
        ...
        print()
        sample_images('cbegan-samples/epoch-{0}.png'.format(epoch + 1))
        np.random.shuffle(data)
except KeyboardInterrupt:
    print()

print('Training complete!')
sample_images('cbegan-samples/final.png')
