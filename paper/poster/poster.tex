...
\begin{tikzpicture}
  ...
  % For LSGAN we interpret the activations of the penultimate dense
  % layer as a source of representations. BiGAN has an explicit inference module and requires no further modification. We find that while GANs
  % are superior to autoencoders, they are not yet competitive with existing
  % classical or deep transfer-learning techniques at this task.
  % };

  \draw (7.75, 1.2) node {\scalebox{0.8}{
  ...
  \draw (4.4, -1.7) node {\scriptsize\textdagger\hspace{0.5pt} Whitening Transform};
\end{tikzpicture}
\vspace{-6mm}
}
...
\vspace{-3mm}
\begin{center}
  \includegraphics[scale=0.174]{qr-code}\hspace{1cm}\includegraphics[scale=0.174]{qr-code}

  \vspace{-3mm}
  \hspace{0.1cm}\texttt{Code}\hspace{2.6cm}\texttt{Paper}
\end{center}

% \includegraphics[scale=0.1]{qr-paper}
}
